...
/************************
 *Draw the sprite described by spriteData structure
 *color is optional, it is applied like a filter, white is default
 *Pre : sprite->Begin() is called
 *Post: sprite->End() is called
 *spriteData.rect defines the porition of spriteData.texture to draw
 * spriteData.rect.right must be the right edge + 1
 * spriteData.bottom must be bottom end + 1
 */
...
void Graphics::drawSprite(const SpriteData &spriteData, COLOR_ARGB color)
{
	...
	if (spriteData.flipHorizontal)
	{
		...
		//Flip coocurs aroudn left edge, translate right to put
		//flipped image in samem location as original.
	...
	if (spriteData.flipVertical)
	{
		...
		//Get cetner of flipped image
		spriteCenter.y -= (float)(spriteData.height*spriteData.scale);
		//flip occurs araoudn top edge, translate down to put
	...
	D3DXMatrixTransformation2D(
		&matrix,           //To store the transfrom matrix
